...
export function Booking() {
  ...
  useEffect(() => {
    const handleSelectService = (e: CustomEvent) => {
      handleChange('serviceId', e.detail);
    };
    window.addEventListener('select-service', handleSelectService as EventListener);
    return () => window.removeEventListener('select-service', handleSelectService as EventListener);
  }, [handleChange]);

  // Fetch booked slots from localStorage when date or master changes (or after new booking)
  useEffect(() => {
    if (!formData.date || !formData.masterId) {
      setBookedSlots([]);
      return;
    }
    try {
      ...
      if (bookingsStr) {
        ...
        const booked = bookings
          .filter((b: any) => b.date === formData.date && b.masterId === formData.masterId)
          .map((b: any) => b.time);
  ...
  }, [formData.date, formData.masterId, status]);
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <FormField label="Дата" id="booking-date" error={errors.date} required>
                    <div className="relative">
                      {/* Native date input overlaid completely transparently to catch taps reliably on iOS/Android */}
                      <input
                        id="booking-date"
                        type="date"
                      ...
                      />
                      {/* Styled visual layer — behind the transparent input */}
                      <div
                        className={`${inputClass} flex items-center justify-between w-full relative z-0`}
                        aria-hidden="true"
                      >
                        ...
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="text-text-muted flex-shrink-0">
                  ...
                  <FormField label="Час" id="booking-time" error={errors.time} required>
                    <div className="relative">
                      ...
                      >
                        {availableTimeSlots.length === 0 && formData.date && formData.masterId ? (
                          <option value="" disabled>Немає вільних годин</option>
                        ) : (

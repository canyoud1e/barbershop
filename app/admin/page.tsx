...
export default function AdminPage() {
  ...
  // Add dummy bookings for demonstration
  const generateDemoBookings = () => {
    ...
    setBookings(demo);
    localStorage.setItem('noir_bookings', JSON.stringify(demo));
  };
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-8 py-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
          ...
          <div className="flex gap-3">
            <Button variant="outline" size="sm" onClick={generateDemoBookings}>
              Генерувати демо
            </Button>
            <Link href="/">
        ...
        {filteredBookings.length === 0 ? (
          <div className="bg-bg-card border border-border rounded-card p-12 text-center">
            <p className="text-text-muted mb-4">Записи не знайдені</p>
            <Button variant="outline" onClick={generateDemoBookings}>
              Створити демонстраційні записи
            </Button>
          </div>
        ) : (
